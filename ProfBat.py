...
class battler:
    ...
    def __init__(self, name, hp, at, df, sp, tp, mvs, btNum):
        self.name = name
        self.hp = hp
        self.at = at
        self.df = df
...
class move:
    ...
    def __init__(self, name, dmg, acc, eat,nat, sto, sts, des):
        self.name = name
        self.dmg = dmg
        self.acc = acc
        self.eat = eat
...
def determineBattlers():
    #All of the moves in the game (so its easier to modify them)
    moves = [
            move("Punch",50,95,' ', ' ',['df',-10],[''],'Issue a mighty punch to the face!'),
            move("Cheat",250,15,'pf','st',[''],['hp',25], "Try to sneak yourself some health, just don't get caught!"), 
            move("Study",0,75,' ',' ',[''],['hp',100], 'Study! Rejuvinate your Health!'), 
            move("Hack",75,50,'wd',' ',['df',-30],[''], 'Hack someones social media. They\'ll have a fun day'), 
            move("No Comments", 25, 100, 'pf', 'st',['sp',-10],[''],"What does this do again?"),
            move("F",25,85,'st','pf',['at',-10],[''],"BOOM! Failure"), 
            move("Homework",40,90,'st','pf',['sp',-10],[''],"Want to slow them down? Give them homework"), 
            move("Cancel Class",0,100,' ',' ',['hp',-10],['hp', 100],"Take a break for yourself!"), 
            move("Group Project!",35,80,'st','pf',[''],[''],"Nothing worse than one student doing all the work"), 
            move('Critque',50,80,'pf',' ',[''],[''],"You should do this better"),
            move('Im the Boss',60,65,'pf','st',[''],[''],"You are literally their boss"),
            move('Java is Javascript',200,10,'st','wd',['at',-20],[''],"I mean, aren't they right?")]

    #All the battlers in the game (easier to modify them here)
    battlers = [battler("Dr.Ford",300,80,50,35,['pf','dc'],[moves[10],moves[5],moves[9],moves[6]],1), 
                battler("Ben",250,95,50,15,['pf'],[moves[6],moves[8],moves[0],moves[5]],2), 
                battler("Jackson",250,50,30,45,['pf'],[moves[11],moves[8],moves[7],moves[3]],3), 
                battler("Markley",400,35,50,35,['pf','wd'],[moves[11],moves[9],moves[5], moves[8]],4),
                battler("Student A",350,50,85,25,['st'],[moves[1],moves[4],moves[2],moves[0]],5), 
                battler("Student B",450,75,30,25,['st'],[moves[3],moves[2],moves[0],moves[11]],6),
                ] 
